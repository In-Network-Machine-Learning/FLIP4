...
def common_parser(fname, config):
    with open(fname, 'a') as parser:
        parser.write("    state parse_ethernet {\n"
                     ...
                     "    state parse_udp { \n"
                     "        pkt.extract(hdr.udp);\n"
                     "        meta.feature0 = (bit < 32 >)hdr.udp.srcPort;\n"
                     "        meta.feature1 =  (bit < 32 >)hdr.udp.dstPort;\n"
                     "        transition accept;\n"
                     "    }\n\n"
                     "    state parse_tcp {\n"
                     "        pkt.extract(hdr.tcp);\n"
                     "        meta.feature0 = (bit < 32 >)hdr.tcp.srcPort;\n"
                     "        meta.feature1 = (bit < 32 >)hdr.tcp.dstPort;\n"
                     "        transition accept;\n"
                     "    }\n\n")
...
def common_logics(fname, config):
    with open(fname, 'a') as ingress:
        ingress.write("        decision_mitigation.apply();\n")
