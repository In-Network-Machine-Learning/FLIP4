...
def Table_to_TCAM(table, code_len):
    ...
    print('Input table has: ',len(separation_value_input),'entries and: ', len(separation_value),' different ranges and output TCAM entry has', len(list(TCAM_table.keys())))

    match = 0
    counts = 0
    correct_match = 0
    keys = list(TCAM_table.keys())
    # separation_value = random.sample(list(table.keys()), 1000)
    separation_value = list(table.keys())
